...
__all__ = ['cov', 'corr', 'CountFrequency', 'CountQuantiles', 'Abs',
           'CountDistribution','Statistic', 'mean', 'exp', 'Sum', 'dot']

def Abs(data):
    try:
        new = list()
        for record in data:
            new.append([abs(value) for value in record])
        return Matrix(data, check=False)
...
def Sum(data):
    ...
    elif isinstance(data, SeriesSet):
...
def dot(matrix_1, matrix_2):
    try:
        if matrix_1.dim.Col != matrix_2.dim.Ln:
            raise ValueError('shapes (%d, %d)'%matrix_1.dim +\
                             ' and (%d, %d) not aligned.'%matrix_2.dim)
        ...
        if len(matrix_1[0]) != len(matrix_2):
            raise ValueError('shapes (%d, %d)'%(len(matrix_1[0]),len(matrix_1))+\
                             ' and (%d, %d) not aligned.'%(len(matrix_2[0]),
                                                           len(matrix_2)))
