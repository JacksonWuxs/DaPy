...
class Table(object):
    ...
    def drop_m_value(self, drop='LINE', symbol=None):
        if drop.upper() == 'LINE':
            new_frame = list()
            Ln = self._dim.Ln
            ...
            self._dim = Table.dims(Ln, self._dim.Col)
            self._Table = new_frame
            del new_frame
    ...
    def pop_col(self, item):
        ...
        pop_name = self._columns.pop(pos) 
        pop_data = list()

        # copy a new Frame
        for record in self._Table:
            try:
                pop_data.append(record.pop(pos))
            except IndexError:
                pass

        self._m_value -= pop_data.count(self._m_symbol)
        self._dim = Table.dims(self._dim.Ln, self._dim.Col-1)
        return (pop_name, pop_data)
...
class DataSet(object):
    ...
    def pop_col(self, item):
        '''Delete and return all the value of this variable.
        Key-word(item) could assignment as a number or the variable name.
        '''
        data = self._data.pop_col(item)
        self._size = self._data.dim
        self._titles = self._data.titles
        return data
...
class SeriesSet(object):
    ...
    def pop_col(self, item):  # OK #
        ...
        pop_name = self._columns.pop(pos)
        pop_data = self._set.pop(item)
        self._m_value.pop(pos)
        self._type_value.pop(pos)
        self._type_column.pop(pos)
        self._dim = SeriesSet.dims(self._dim.Ln, self._dim.Col-1)
        return (pop_name, pop_data)
        ...
        with open(addr) as f:
            ...
            for m, record in enumerate(reader):
                for i, l in enumerate(col):
                    datas[i].append(transform(l, record[l]))
