...
from _numeric import describe, mean, _sum
...
class SeriesSet(object):
    ...
    def __init_dict(self, series, miss_symbol):
        ...
        if not self._columns:
            ...
            self._columns.extend(['Col_%d'%i for i in range(len(series) - len(set(self._columns)))])
        elif len(set(self._columns)) > len(series):
    ...
    def __init_normal(self, series, miss_symbol, columns, element_type):
        ...
        if is_seq(series[0]):
                    ...
                    for i, value in enumerate(self._columns):
                        if self._columns.count(value) != 1 and \
                           self._columns.index(value) != i:
                            self._columns[i] = 'Col_%d'%i
                    self._columns.extend(['Col_%d'%i for i in \
            ...
            if not all(self._type_value):
                for i, value in enumerate(self._type_value):
                    if value is None:
                        self._type_value[i] = type(None)
    ...
    def __init__(self, frame=None, columns=None, element_type_=None,
        ...
        # Check the type of frame is legal
            ...
            if not element_type_:
                self._type_value = [None for item in self._Frame[0]]
                for item in self._Frame:
                    for i, value in enumerate(item):
                        if self._type_value[i] is not None:
                            continue
            ...
            if len(set(self._columns)) < dim_Col:
                for i, value in enumerate(self._columns):
                    if self._columns.count(value) != 1 and self._columns.index(value) != i:
                        self._columns[i] = 'Col_%d'%i
                self._columns.extend(['Col_%d'%i for i in range(len(self._columns),
                                                                dim_Col)])
            elif len(set(self._columns)) > dim_Col:
                self._columns = self._columns[:dim_Col]
...
class Table(object):
    ...
    def __init__(self, table=None, columns=None, miss_value=None):
        ...
        if table == None and columns == None:
            ...
            dim_col = len(max(table, key=len))
            if columns is None:
                self._columns = ['Col_'+str(i) for i in range(dim_col)]
            else:
                self._columns = [str(i) for i in columns]
                if len(self._columns) < dim_col:
                    for i in range(dim_col - len(self._columns)):
                        self._columns.append('Col_'+str(i))
                elif len(self._columns) > dim_col:
                    self._columns = self._columns[:dim_col]
    ...
    def append_col(self, variable_name, series):
