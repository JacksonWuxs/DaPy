...
上面那个问题的答案就是， <u>*DaPy专为数据分析师设计，而不是程序员.*</u>  DaPy的用户只需要关注于他们解决问题的思路，而不必太在意数据结构这些编程陷阱。这些大量编程陷阱出现的原因分为两方面，其一，API的设计较为严格，符合程序开发的要求，但不太适合数据分析师的学习。其二，Pandas在设计时以列存数据，并且没有进行行优化的情况下，导致了这个极差的操作体验。下面，我们将具体讨论，DaPy在这两个问题上的表现。
...
上面这个讨论的典型案例是Pandas不支持对于`DataFrame.iterrows()`迭代出来的行进行操作。DaPy以视图的方式，让人们又可以轻松地按照符合人们习惯的方式处理数据了。下面是我们对于“行视图”对象的一些案例。
